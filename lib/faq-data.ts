...
Settlement is per slice, not per job. A run that dies halfway bills for the slices that completed and nothing more, and cancelling is billed the same way. A job rejected before any GPU work, whether a token count that does not reconcile, a dataset that fails its content scan, or a host that turns out to be busy, settles to zero and frees your deposit rather than stranding it.
...
• Reloading a chat that used an adapter will refuse it, and you will be asked to start a new chat. Serve-back cannot be resumed across a reload, and the host refuses rather than quietly serving you the base model.
...
The training itself runs in a separate trainer sidecar, reached over a Unix domain socket and never exposed to a network. It counts tokens, scans content, and runs the loop, and nothing else. That split is deliberate: the process touching money is not the process running arbitrary user data through a GPU.
...
• Datasets are removed on every terminal path and swept at start-up, so you hold a client's corpus in clear only for the length of the run, and only on the staging volume.
